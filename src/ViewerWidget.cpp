...
void ViewerWidget::generateUVSphere(double radius, int slices, int stacks)
{
	...
	for (int i = 0; i < stacks; ++i) {
		for (int j = 0; j < slices; ++j) {
			...
			// Indexy ötyroch susediacich bodov na mrieûke sfÈry
			int first = (i * (slices + 1)) + j;
			int second = first + slices + 1;

			// Prv˝ trojuholnÌk zo ötvorca (quadu)
			Triangles t1;
			t1.v1 = first;
			t1.v2 = second;
			t1.v3 = first + 1;
			...
			// Druh˝ trojuholnÌk zo ötvorca (quadu)
			Triangles t2;
			t2.v1 = first + 1;
			t2.v2 = second;
			t2.v3 = second + 1;
...
void ViewerWidget::writeSphereVTK(const std::string& filename)
{
	...
	// HlaviËka VTK form·tu pre PolyData
	file << "# vtk DataFile Version 3.0\n";
...
{
	// 1. Prevod uhlov zo stupÚov na radi·ny
	double phi = azimut * M_PI / 180.0;
	double theta = zenit * M_PI / 180.0;

	// 2. V˝poËet smerov˝ch vektorov kamery (b·za priemetne)
	// Smer pohæadu
	...
	// Vektor hore (v) - zÌskan˝ vektorov˝m s˙Ëinom
	double vx = uy * nz - uz * ny;
	double vy = uz * nx - ux * nz;
	double vz = ux * ny - uy * nx;

	// 3. Transform·cia 3D bodu do s˙radnÌc kamery
	double x_cam = v.x() * ux + v.y() * uy + v.z() * uz;
	double y_cam = v.x() * vx + v.y() * vy + v.z() * vz;
	double z_cam = v.x() * nx + v.y() * ny + v.z() * nz;

	TransformedPoint tp;
	tp.z = z_cam; // OdloûÌme hÂbku

	// 4. Projekcia na 2D obrazovku
	if (project == 0) {
		// RovnobeûnÈ premietanie
		tp.screen.setX(static_cast<int>(centerX + x_cam));
		tp.screen.setY(static_cast<int>(centerY - y_cam)); // -y, pretoûe v poËÌtaËovej grafike ide os Y dole
	}
	else {
		// StredovÈ premietanie (perspektÌva)
		// d / (d - z) zabezpeËÌ, ûe vzdialenejöie objekty s˙ menöie
...
void ViewerWidget::drawWireFrame(double azimut, double zenit, int project, double distance, QColor color)
{
	...
	// VytvorÌme si pole pre pretransformovanÈ 2D body
	std::vector<QPoint> screenPoints(sphereVerticles.size());

	// Pretransformujeme kaûd˝ jeden 3D bod na obrazovku
	...
	// VykreslÌme hrany trojuholnÌkov pomocou tvojho Bresenhama
